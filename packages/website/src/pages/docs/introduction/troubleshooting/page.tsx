...
function FormatErrorsContent(isZh: boolean) {
	return (
		<ol className="list-decimal pl-5 space-y-4 text-gray-700 dark:text-gray-300">
			<li>
				<strong>
					{isZh ? "确认模型是否支持" : "Verify model compatibility"}
				</strong>
				<p className="mt-1">
					{isZh
			...
			<li>
				<strong>
					{isZh
						? "检查代理/网关的参数转发"
						: "Check proxy/gateway parameter forwarding"}
				</strong>
				<p className="mt-1">
					{isZh
	...
	return (
		<>
			...
			<ol className="list-decimal pl-5 space-y-4 text-gray-700 dark:text-gray-300">
				<li>
					<strong>
						{isZh ? "先从简单指令开始" : "Start with a simple instruction"}
					</strong>
					<p className="mt-1">
						{isZh
				...
				<li>
					<strong>
						{isZh ? "尝试最强模型" : "Try the strongest model available"}
					</strong>
					<p className="mt-1">
						{isZh
				...
				<li>
					<strong>
						{isZh ? "优化指令质量" : "Improve instruction quality"}
					</strong>
					<p className="mt-1">
						{isZh
				...
				<li>
					<strong>
						{isZh ? "提供充足的上下文" : "Provide sufficient context"}
					</strong>
					<p className="mt-1">
						{isZh
				...
				<li>
					<strong>
						{isZh ? "检查 HTML 清洗结果" : "Check HTML sanitization output"}
					</strong>
					<p className="mt-1">
						{isZh
...
function WrongElementContent(isZh: boolean) {
	return (
		<ol className="list-decimal pl-5 space-y-4 text-gray-700 dark:text-gray-300">
			...
			<li>
				<strong>
					{isZh ? "检查目标元素类型" : "Check target element type"}
				</strong>
				<p className="mt-1">
					{isZh
  ...
  customFetch: async (url, init) => {
    ...
    return fetch(url, { ...init, body: bodyStr })
			...
			<p className="text-sm">
				...
				{isZh
					? " 了解 customFetch 的完整用法。"
					: " for full customFetch documentation."}
			</p>
		</div>
	);
}
...
const SYMPTOM_COLORS = {
	...
	amber:
		"border-amber-400 bg-amber-50 dark:bg-amber-900/15 text-amber-800 dark:text-amber-200",
	orange:
