...
LLM_MODEL_NAME="qwen3.5-plus"`}
			...
			<section className="mb-10">
				...
				<p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
					{isZh ? (
						<>
							如果你计划将它集成到你的 Web 应用中，建议搭建一个后端代理来转发
							LLM 请求，并使用 <code>customFetch</code> 携带 Cookie
							或其他鉴权信息：
						</>
					) : (
						<>
							If you plan to integrate it into your web app, it's better to have
							a backend proxy for the LLM and use <code>customFetch</code> to
							authenticate the request with cookies or other methods:
						</>
					)}
				</p>
				<CodeEditor
			...
			<section className="mb-10">
				...
				<p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
					{isZh ? (
						<>
							一些 LLM
							能从主动缓存中受益很多。由于各个供应商的主动缓存接口不同，推荐使用{" "}
							<Link
						...
						<>
							Some LLMs benefit significantly from prompt caching. Because each
							provider exposes caching differently, use{" "}
							<Link
								href="/advanced/page-agent-core#configuration"
								className="text-blue-600 dark:text-blue-400 hover:underline"
							>
